...
DEFAULT_REPOSITORY = "bash0816/Codex-Termux"
...
def resolved_v8_crate_version(source_root: Path) -> str:
    cargo_lock = tomllib.loads((source_root / "codex-rs" / "Cargo.lock").read_text())
    versions = sorted(
        {
            package["version"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch public rusty_v8 Android artifacts for upstream Codex probes."
    )
    parser.add_argument(
        "--repository",
    ...
    parser.add_argument(
        "--source-root",
        default=str(ROOT / "source-repo"),
        help="Path to the upstream source checkout root that contains codex-rs/Cargo.lock.",
    )
