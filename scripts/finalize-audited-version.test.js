...
test('finalize: previous_stable_version becomes the newly finalized version', () => {
  const manifest = createManifest();
  const updated = computeFinalizedManifest(manifest, '0.145.0');
  assert.equal(updated.previous_stable_version, '0.145.0');
});
