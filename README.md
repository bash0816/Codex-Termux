...
**This bug is patched in `@bash0816/codex-termux@0.139.0` and later.** The upstream fix is tracked in [openai/codex#26277](https://github.com/openai/codex/issues/26277).

アップストリームの `codex exec` が Android で `lock() not supported` クラッシュする不具合は、`@bash0816/codex-termux@0.139.0` 以降で修正済みです。
